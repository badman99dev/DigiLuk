...
class DashboardScreen extends ConsumerWidget {
            ...
            builder: (context, snapshot) {
              ...
              if (!snapshot.hasData || snapshot.data!.isEmpty) {
                return EmptyState(
                  title: 'No Trusts Yet',
                  subtitle:
                      'Create your first trust to start managing funds transparently.',
                  icon: Icons.account_balance_outlined,
                  onAction: () {
                    Navigator.pushNamed(context, CreateTrustScreen.routeName);
                  },
                  actionLabel: 'Create Trust',
                );
              }
              final trusts = snapshot.data!;
              return ListView.builder(
                padding: const EdgeInsets.all(16),
                itemCount: trusts.length + 1,
                itemBuilder: (context, index) {
                  if (index == 0) {
                    return _buildSummaryCard(trusts);
                  }
                  final trust = trusts[index - 1];
                  return _buildTrustCard(context, trust);
                },
              );
            },
          );
  ...
  Widget _buildSummaryCard(List<TrustModel> trusts) {
    double totalBalance =
        trusts.fold(0, (sum, trust) => sum + trust.totalBalance);
    return Container(
      ...
      child: BalanceCard(
        ...
        totalIncome: 0,
        totalExpense: 0,
        trustName: 'Total Balance (${trusts.length} Trusts)',
      ),
    );
  }

  Widget _buildTrustCard(BuildContext context, TrustModel trust) {
    IconData typeIcon;
    switch (trust.type) {
